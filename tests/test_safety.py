...
class Sanitizer(unittest.TestCase):
    def test_drops_credentials_and_unknown_fields(self):
        cleaned = sanitize_fields({
            "username": "amy@example.com",   # allowed? no — not whitelisted
...
class StorageRefusesCredentials(unittest.TestCase):
    def test_record_event_rejects_credential_shaped_extra(self):
        ...
        import sqlite3
        from phishguard import db, create_app

        app = create_app({"DATABASE": ":memory:"})
        with app.app_context():
            # Patch record_event's insert to attempt a credential column via
            # the guard helper, proving the layer raises rather than stores.
            conn = db.get_db()
            conn.execute(
                "CREATE TABLE IF NOT EXISTS forbidden_demo (password TEXT)"
            )
            ...
            def attempt(payload: dict):
                ...
                conn.execute("INSERT INTO forbidden_demo (password) VALUES (?)", ("x",))

            with self.assertRaises(CredentialStorageError):
                attempt({"password": "canary"})

            # And nothing was written.
            count = conn.execute("SELECT COUNT(*) AS n FROM forbidden_demo").fetchone()["n"]
    ...
    def test_submitted_password_never_reaches_disk(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with app.app_context():
                ...
                cid = db.create_campaign("Canary drill", "acme_webmail", consent=True)
                pid = db.create_participant(cid, "Amy Pond", "amy@example.com", consent=True)

            client = app.test_client()
            path = f"/sim/{cid}/{pid}"  # sim.landing route, static shape
            ...
            # DB contains the event row but zero credential-shaped columns.
            with app.app_context():
                from phishguard import db
                events = db.get_db().execute(
                    "SELECT event_type FROM events WHERE campaign_id = ?", (cid,)
                ).fetchall()
                self.assertEqual(
